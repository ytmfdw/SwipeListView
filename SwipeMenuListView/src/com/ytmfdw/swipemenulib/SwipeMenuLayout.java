...
public class SwipeMenuLayout extends FrameLayout {
	...
	public boolean hasChild() {
		...
		return mMenuView.getItemCount() != 0;
	}
	...
	public boolean onSwipe(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			LogUtil.d("SwipeMenuLayout action up");
			// 只要大于屏幕的一半即可,同时没有打开右滑菜单
			if ((!isOpen())
					&& (isFling || Math.abs(mDownX - event.getX()) > (mMenuView
							.getWidth() / 2))
					&& Math.signum(mDownX - event.getX()) == mSwipeDirection) {
				LogUtil.d("SwipeMenuLayout open");
				// open
				smoothOpenMenu();
			} else {
				LogUtil.d("SwipeMenuLayout close");
				// close
				smoothCloseMenu();
				return false;
			}
